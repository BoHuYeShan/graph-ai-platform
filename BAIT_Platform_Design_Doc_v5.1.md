# B.A.I.T. 平台完整设计文档 v5.1

> **Bureau of Advanced Interdisciplinary Theories**  
> 一个安全优先的虚构学术文稿工坊、社区小说档案库与 ARG 世界观共创平台。  
> v5.1 的核心目标是：在 v5 安全优先框架上恢复更精确的艺术边界——保留 paper 写作体验、多入口世界观、社区共创、解谜乐趣、讽刺性发布卡与渐进信息深度，同时避免真实人物误认、主动骚扰、现实学术背书误用、截图传播误导与跨法域法律风险。

---

## v5.1 总原则

v5.1 不再把“让现实人物相信自己被真实期刊收录”作为产品目标。平台可以提供强烈的学术排版、审阅流程、虚构机构世界观和社区共创沉浸感，但这些沉浸感必须建立在可见、持续、可截图、可 OCR、可翻译的披露基础上。

相对 v5，v5.1 细化以下边界：

1. **未经知情同意的真实人物**只能作为 Source Subject；**自愿、知情、可追溯同意的投稿者**可以使用笔名作为 Article Author。
2. 披露必须存在于每个入口和每篇文章，但**披露深度可以随入口变化**：公开入口短标签，社区入口来源上下文，档案入口完整世界观说明。
3. 公开入口禁用假分区标签；档案入口可以把假分区标签作为“解谜展品”展示，但必须标注其为虚构叙事标签。
4. Release Card 可以保留证书式视觉讽刺，但正面必须醒目标明其不是学术发表、收录或认证证明。
5. 二维码应先进入通用验证页并首屏披露平台性质；输入编号后可显示针对该编号的具体虚构文稿说明。
6. ARG 可以使用双关、零宽字符、SVG、隐写图等反翻译/难翻译技巧；但这些技巧不得用于免责声明、平台性质披露、来源说明或真实人物未授权未背书说明。

核心判断如下：

1. **B.A.I.T. 不是现实学术期刊、预印本服务器、收录机构或认证机构。**
2. **平台内容只能经过社区编辑审核、内容边界审核、风格审核和世界观一致性审核，不构成现实学术同行评审。**
3. **未经知情同意的真实人物不能作为站内论文作者、投稿者、被收录者或认证对象。**
4. **未经同意的真实人物只能作为公开材料背景对象（Source Subject / Public Context）出现，并且必须明确未参与、未授权、未背书。**
5. **自愿、知情、可追溯同意的投稿者可以使用笔名作为 Article Author，但必须保留同意记录、许可记录、撤稿/改名通道。**
6. **所有入口都必须能让正常用户主动发现平台性质；入口差异只体现在披露深度、叙事深度和交互复杂度，不体现在真假披露的有无。**
7. **用户协议、冷门页面、源码注释、PDF 元数据和 ARG 谜题只能作为补充披露，不能承担主要告知责任。**
8. **禁止任何社区成员主动联系、挑衅、诱导、嘲讽、围攻现实人物。**
9. **截图、PDF、分享卡片必须自带上下文水印，避免脱离原页面后被误用。**
...
1. [v5.1 项目定位](#1-v51-项目定位)
2. [从 v1-v4 到 v5.1 的关键调整](#2-从-v1-v4-到-v51-的关键调整)
...
14. [Front Matter v5.1 规范](#14-front-matter-v51-规范)
...
20. [CAST / HOOK / CATCH / RELEASE v5.1 解释](#20-cast--hook--catch--release-v51-解释)
...
B.A.I.T. v5.1 是一个由三类内容共同组成的虚构学术创作系统：
...
v5.1 的核心叙事仍可保留“学术机构表皮”和“跨维度档案”的世界观，但平台不能向任何现实用户暗示：其内容已经获得真实期刊发表、学术收录、权威认证、现实同行评审或机构背书。
...
v5.1 并不取消沉浸式体验，而是改变沉浸来源：

| 沉浸来源 | 保留 | 调整 |
|---|---|---|
| 学术排版 | 保留 | 页面标签必须标明虚构文稿 |
| 审核流程 | 保留 | 改为社区编辑审核，不称现实同行评审 |
| 虚构作者 | 保留 | 不映射未经同意的真实人物 |
| 自愿投稿者 | 有条件保留 | 知情同意、笔名优先、可追溯、可撤稿/改名 |
| 真实人物背景 | 有条件保留 | 未经同意者只能作为 Source Subject / Public Context |
| PDF 导出 | 保留 | 每页带上下文水印 |
...
| 未经同意把真实人物作为论文作者 | 删除。未经知情同意的真实人物只能作为 Source Subject。 |
| 自愿投稿者作为作者 | 保留例外。自愿、知情、可追溯同意者可用笔名作为 Article Author。 |
| 假 JCR / CAS 分区 | 公开入口删除；档案入口可作为“虚构标签展品”保留，并必须标注为世界观叙事。 |
| 真实收录证书 / 录用证明 | 删除。改为 Fictional Release Card / Archive Display Sheet。 |
| 验证码输入后才揭示为假 | 改为验证页首屏先披露平台性质；输入编号后显示该编号对应的具体虚构文稿说明。 |
...
v4 的核心张力是“真实感 vs 可发现真相”。v5.1 的核心原则改为：

> 真实感来自流程、排版、术语和世界观；安全性来自显性标签、来源分离、社区公约和导出物水印。不得通过隐藏事实制造真实感。

v5.1 允许保留“渐进信息深度”：用户可以从短标签、页脚说明、Source Context、档案入口和术语页逐层看到更多解释。但它不允许“渐进事实披露”：平台是否虚构、是否为现实期刊、真实人物是否为作者、社区审核是否等于同行评审，这些事实不能只在深层页面才出现。
...
v5.1 不把 B 组定义为“被钓对象”，而定义为“需要被保护的普通访客”。他们可以获得完整的 paper 写作和阅读体验，但不能被诱导相信平台提供现实学术背书。
...
D 组是真实存在、主动参与、明确知情并同意平台性质的投稿者。D 组与 C 组的区别在于：D 组主动提交内容并完成了可追溯的同意流程；C 组只是被公开材料提及或被社区评论。

D 组可以作为 Article Author，但默认应使用笔名或社区身份。若投稿者坚持使用真实姓名，必须额外确认以下事项：

1. 他理解 B.A.I.T. 是虚构文稿工坊，不是真实期刊、预印本服务器或认证机构。
2. 他理解站内发布不构成现实学术发表、收录、同行评审或机构背书。
3. 他拥有提交内容的权利，并接受指定许可。
4. 他可以请求撤稿、改名、改用笔名或删除可识别信息。
5. 维护者保留因安全、误认或骚扰风险拒绝真实姓名展示的权利。
...
| voluntary_submission | 自愿投稿文稿 | 是 | 可含知情同意投稿者 |
...
自愿投稿文稿来自主动参与者。它与 Source Subject 最大区别是：投稿者本人完成了知情同意流程并主动提交内容。

自愿投稿文稿必须满足：

- 投稿者主动提交，而非社区替其整理。
- 投稿者勾选短声明，不只是在长用户协议中默认同意。
- 投稿者默认使用笔名；真实姓名展示需要二次确认。
- 投稿者可请求撤稿、改名或隐藏身份。
- 文稿仍显示 `Community Manuscript / Not Journal Publication`。
...
未经知情同意的真实人物只能作为 Source Subject 出现，不能作为：
...
知情同意的自愿投稿者不适用本禁令，但必须进入 `voluntary_submission` 流程，并使用独立的同意记录与作者展示规则。
...
自愿投稿者可以作为 Article Author，但应遵循“笔名优先、真实身份最小化、可撤回”的原则。

| 项目 | 规则 |
|---|---|
| 作者显示 | 默认显示笔名或社区 ID |
| 真实姓名 | 仅在投稿者二次确认后显示 |
| 同意记录 | 保存 Issue/PR 勾选记录、时间戳、版本化条款 |
| 许可 | 明确 CC-BY-SA 或其他兼容许可 |
| 撤稿/改名 | 提供低摩擦渠道 |
| 审核 | 仍需内容安全、格式、来源和误认风险审核 |

投稿确认短声明必须独立显示：

> 我理解 B.A.I.T. 是社区虚构文稿工坊，不是真实学术期刊、预印本服务器、收录机构或认证机构；我的投稿不会获得现实学术发表、同行评审、收录或机构背书。
...
disclosure_level: short_visible
...
voluntary_submission:
  is_voluntary: false
  consent_record: null
  author_display_preference: fictional_profile
...
### 14.2 自愿投稿者 Front Matter

```yaml
content_mode: voluntary_submission
article_authors:
  - name: "Null Observer"
    type: voluntary_pseudonym
    consent_record: "github-pr-42-checkbox-v2026-06-16"
    real_name_display: false

voluntary_submission:
  is_voluntary: true
  consent_record: "github-pr-42-checkbox-v2026-06-16"
  consent_statement_version: "bait-consent-v5.1"
  author_display_preference: pseudonym
  understands_fictional_workshop: true
  understands_not_journal_publication: true
  understands_not_peer_reviewed: true
  understands_not_certification: true
  license: "CC-BY-SA-4.0"
  withdrawal_contact: "takedown@placeholder.example"
```

自愿投稿者字段用于区分“本人主动投稿”与“社区基于公开材料整理”。没有 `is_voluntary: true` 和可追溯 `consent_record` 的真实人物，不得进入作者栏。
...
v5.1 使用三层披露：短披露、上下文披露、完整披露。

三层披露不是为了让部分用户看不见真相，而是为了在不破坏阅读体验的前提下提供不同深度的信息。最低层短披露必须出现在文章页可见区域；完整披露可以放在页脚、About、政策文件和验证页中。
...
- 公开入口使用短标签：`社区虚构文稿`。
- 社区入口展开 Source Subject、审核范围和社区规则。
- 档案入口展开 R.E.E.F. 世界观、分级映射和 ARG 结构。

不允许：

- 公开入口完全不披露。
- 只在页脚或用户协议披露。
- 让平台性质只能通过 ARG、源码或术语页发现。
- 不同入口对“是否为现实期刊”给出不同暗示。

---

## 16. 截图水印与分享防误用

截图传播是 v5.1 的核心风险之一。用户可能只看到截图，不看到页脚、用户协议或完整说明。因此页面必须提供截图上下文水印。
...
QR 码直接跳转到验证页，验证页第一屏显示：

> B.A.I.T. 是社区虚构文稿工坊，不是真实期刊或认证机构。

不得要求输入验证码后才显示平台性质。输入编号后，可以显示针对该编号的具体说明，例如：

> 此编号对应一份 B.A.I.T. 社区虚构文稿展示卡，不是学术发表记录、同行评审记录、收录记录或认证记录。
...
- 验证页首屏先说明平台不是现实出版机构。
- 输入 DIO / Release Card 编号后显示该编号对应的文稿标题、站内链接和“虚构文稿展示物”说明。
- 显示该卡片是否由站内生成，以防伪造截图或站外冒充。
- 对未命中的编号给出“非站内档案”说明，但不得写成“未收录”“认证失败”或“无效证书”。

不允许：

- 使用“验证通过”“认证有效”“收录状态有效”等措辞。
- 让验证页看起来像现实期刊的证书验证系统。
- 只在输入编号后才揭示平台是虚构项目。

### 18.4.2 编号查询结果文案

编号查询页必须避免使用现实认证系统语气。推荐状态如下：

| 查询结果 | 推荐文案 | 禁用文案 |
|---|---|---|
| 命中站内生成记录 | 此编号对应一份 B.A.I.T. 站内社区虚构文稿展示卡，不是学术发表、收录、认证或同行评审记录。 | 验证通过、认证有效、已收录 |
| 编号未命中 | 此编号不在 B.A.I.T. 站内档案中。B.A.I.T. 不是真实认证机构，不对外部材料提供验证。 | 未收录、未认证、证书无效 |
| 编号格式错误 | 该输入不符合 B.A.I.T. 站内虚构档案编号格式。B.A.I.T. 编号不是 DOI、证书号或学术注册号。 | 编号非法、认证失败 |
| 外部图片或截图冒充 | 无法仅凭外部图片确认其来源。请以 B.A.I.T. 站内页面和首屏披露为准；B.A.I.T. 不对站外生成材料提供背书。 | 假证、伪证、无效认证 |

未命中状态的核心目的不是证明“某个认证不存在”，而是切断站外伪造图片与 B.A.I.T. 的关系，并再次说明平台本身不提供现实认证。
...
Release Card 可以借用证书的构图、边框、排版和仪式感来讽刺学术凭证制度，但必须满足：

- 不使用 Certificate / Acceptance / Certified / Indexed 等名称。
- 不使用真实大学、出版社、政府、学会的印章式图案。
- 正面醒目标明 `社区虚构文稿展示卡 · 不是学术发表证明`。
- 截图时仍能看到 `Not an academic certificate` 角标。
- QR 码跳转到首屏披露平台性质的验证页。
- 不能被单独裁剪后看起来像现实证书。
...
披露层必须手机/电脑一致；ARG 层可以根据设备改变交互方式。手机端无法稳定使用“查看源代码”或 DevTools，因此可提供等效谜题入口：
...
这些差异只能用于世界观谜题，不能用于隐藏平台性质、免责声明或 Source Subject 说明。

---

# 第五部分：审核流程与社区治理

## 20. CAST / HOOK / CATCH / RELEASE v5.1 解释

v5.1 保留世界观术语，但每个术语旁边必须有现实解释。
...
公开入口不得使用 JCR Q1、CAS 一区、SCI 收录等现实学术分区或收录标签。档案入口可以展示这些标签的“虚构版本”，但只能作为对欺骗机制本身的揭露、评论和世界观展品。

推荐写法：

```text
Former portal-facing fictional label: Category S — Landmark Contribution
Archive interpretation: This was a fictional prestige label used inside the B.A.I.T. narrative layer.
This label is not a JCR, CAS, SCI, journal, or institutional classification.
```

中文说明：

> 此标签为 B.A.I.T. 世界观中的虚构分级展品，不是现实 JCR、中科院分区、SCI 收录、期刊评价或机构认证。

禁止在档案入口将此类标签重新包装成可截图误用的荣誉标识；它必须以“展品/揭露/注释”的形式出现。
...
v5.1 不使用反翻译技术隐藏披露。可以使用 SVG、Unicode、零宽字符作为 ARG 谜题，但不能用于免责声明、来源说明、未授权未背书说明。
...
v5.1 的核心不是削弱 B.A.I.T. 的世界观，而是让世界观从“可能误导现实人物的伪装系统”转向“诚实、可持续、可共创的虚构学术创作系统”。
...
*本文档为 B.A.I.T. v5.1 安全优先总设计文档。法律相关内容仅为风险框架，不构成法律意见。公开上线前应进行专业法律咨询和小范围受控测试。*
